...
public static partial class BranchNamingTool
{
    ...
    public static string GenerateBranchName(
        ...
        string issueType)
    {
        var numberMatch = CreateTicketNumberPattern().Match(ticketDescription);
        ...
        var description = ticketDescription.Substring(numberMatch.Index + numberMatch.Length).Trim();
        
    
        var formattedDescription = RemoveSpecialCharacters().Replace(description, "");
        formattedDescription = ReplaceSpacesWithUnderscoresRegex().Replace(formattedDescription, "_");
